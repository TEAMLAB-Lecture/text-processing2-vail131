...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    return ' '.join([num[x] for x in input_string if x in num.keys()])
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
        ...
        Examples:
            ...
            "alreadyCamel"
    """
    no_underscore_str=underscore_str.strip('_')
    if no_underscore_str.count('_')==0 :return no_underscore_str
    else :
        no_underscore_str=' '.join(underscore_str.lower().replace('_',' ').split())
        camelcase_str=""
        flag=False
        for c in no_underscore_str :
            if c==' ' : flag=True 
            elif flag :
                flag=False
                camelcase_str+=c.upper()
            else : camelcase_str+=c
